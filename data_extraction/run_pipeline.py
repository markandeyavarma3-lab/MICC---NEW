...
DAILY_PHASES = [
    ...
    ("ann_tags",    "events/tag_announcements.py",             "Announcement taxonomy tags",            None, 180),
...
WEEKLY_PHASES = [
    ...
    ("annual_fin",     "events/fetch_annual_financials.py",    "Annual financials (yfinance)",          None, 21600),
    ...
    ("shp",            "events/fetch_shp.py",                  "BSE shareholding patterns (Part 4 Stage 1)", ["--quarters", "8", "--budget-min", "80", "--notify"], 5400),
    ("bse_scrips",     "registry/fetch_bse_scrip_master.py",   "BSE scrip master incl. Delisted/Suspended (Part 4 Stage 3)", None, 300),
    ("shp_universe",   "registry/build_shp_pit_universe.py",   "Survivorship-free SHP PIT universe re-join (Part 4 Stage 3)", None, 900),
    ("value_gate",     "common/backtest_value_quality.py",     "Value/quality re-backtest gate (cap-lift decider)", None, 900),
...
def main():
    if "--check" in sys.argv:
        run("common/check_db_health.py", "DB Health Check", timeout=300)
        return
    ...
    run("common/check_db_health.py", "Health Check", timeout=300)
